...
DAI can meet with other election boards so that the script fully meets their needs.  The script would require very few changes.  Specifically the changes would inlcude: 1) data source file - Each election board would have their own raw data file with the individual votes to provide.  This code would need to be changed to bring in the new file.  2) reading the raw file - Each board may have different formats to their raw data and some small changes may be needed so that the data is read from this file correctly.  
